...
namespace nsNNLS {
  ...
  public:
    ...
    nnls(const nnls & model)= delete;
    // copy assignment - prevent copying
    nnls & operator=(const nnls & model)= delete;

    // move assignment operator
    nnls & operator=(nnls && model) noexcept {

        if (&model == this){
            return *this;
        }
        std::swap(x, model.x);
        std::swap(x0, model.x0);
        std::swap(oldx, model.oldx);
        std::swap(g, model.g);
        std::swap(oldg, model.oldg);
        std::swap(xdelta, model.xdelta);
        std::swap(gdelta, model.gdelta);
        std::swap(refx, model.refx);
        std::swap(refg, model.refg);
        std::swap(A, model.A);
        std::swap(b, model.b);
        std::swap(ax, model.ax);
        std::swap(fset, model.fset);
        fssize = model.fssize;

        maxit = model.maxit;               // maximum num. iters
        M = model.M;                    // max num. of null iterations
        decay = model.decay;               // parameter to make diminishing scalar to decay by
        beta = model.beta;                // diminishing scalar
        pgtol = model.pgtol;               // projected gradient tolerance
        sigma = model.sigma;               // constant for descent condition

        return *this;
      }

      nnls (nnls && model) noexcept {
          *this = std::move(model);
      }
